...
const form = useForm({
  ...
  contact_information: [],
  client_id: props.accounts?.[0]?.client_id || '',
  next_actions: "",
  expected_sale_value: "",
  ...
  populatePresaleOptions() {
    switch (this.presale_type) {
      ...
      case "2": // Tenders
        this.presale_options = [
          {key: 'submissionDeadline', type: 'date', value: ''},
          {key: 'tenderLink', type: 'text', value: ''},
        ];
        break;
      case "3": // RFQ
        this.presale_options = [
      ...
      case "4": // other
          this.presale_options = [
            {key: 'Brief description', type: 'text', value: ''},
          ];
          break;
      default:
        this.presale_options = [];
...
<template>
  <AppLayout :title="passtitle">
    <PageHeader :title="passtitle" :name="passtitle" />
    ...
    <div class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
            <div class="relative flex-1 max-w-md">
                ...
                <input type="text" v-model="filters.global" placeholder="Search stage accounts by name or solution..."
                ...
                <table class="min-w-full text-xs text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="biz in filteredClients" :key="biz.id"
                            ...
                            <td class="px-6 py-4">
                                <div class="min-w-[180px] space-y-1.5 text-[10px]">
                                    <div v-for="(item, index) in safeParseJSON(biz.latest_stage?.meta)" :key="index" class="p-1 rounded bg-gray-50 border border-gray-100 flex flex-col gap-0.5">
                                        <strong class="text-gray-500 font-bold uppercase text-[7px] tracking-wider">{{ item.key.replace(/_/g, ' ') }}:</strong>
                                        <template v-if="Array.isArray(item.value)">
                                            <div class="flex flex-col gap-1 pl-1.5 mt-0.5">
                                                <div v-for="subItem in item.value" :key="subItem.key" class="flex items-center gap-1 text-[9px]">
                                                    <template v-if="item.key === 'Documents'">
                                                        <Icon icon="flat-color-icons:document" class="h-3 w-3 flex-shrink-0" />
                                                        <a :href="subItem.value" target="_blank" class="text-primary font-bold hover:underline truncate">{{ subItem.key }}</a>
                                                    </template>
                                                    <template v-else>
                                                        <span class="text-gray-400 font-medium">{{ subItem.key.replace(/_/g, ' ') }}:</span>
                                                        <span class="font-bold text-gray-800">{{ subItem.value }}</span>
                                                    </template>
                                                </div>
                                            </div>
                                        </template>
                        ...
                        <tr v-if="filteredClients.length === 0">
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="max-w-xs mx-auto">
                                    ...
                                    <p class="text-gray-900 font-bold text-sm">No Accounts Found for this Stage</p>
    ...
    <DialogModal :show="updateStageModal" @close="closeUpdateStageModal">
      <template #title>Update Business Stage</template>
      <template #content>
        <div class="space-y-4">
          <div>
          ...
          <div v-if="Number(form.project_stage_id) === 2">
            <InputLabel value="Business Solution Title" />
            <TextInput v-model="form.solution_name" type="text" class="mt-1.5 block w-full text-xs" />
            <InputError :message="form.errors.solution_name" class="mt-1" />
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div v-if="showSolutionCategoryX">
              <InputLabel value="Solution Category" />
              <select v-model="form.solution_type_id" class="w-full mt-1.5 border-gray-200 focus:border-primary focus:ring-primary/10 rounded-lg text-xs py-2">
            ...
            <div v-if="showSolutionFieldX">
              <InputLabel value="Select Solution" />
              <select v-model="form.solution_id" class="w-full mt-1.5 border-gray-200 focus:border-primary focus:ring-primary/10 rounded-lg text-xs py-2">
            ...
            <div v-if="showSolutionSubtypeFieldX">
              <InputLabel value="Solution Subtype" />
              <select v-model="form.solution_subtype_id" class="w-full mt-1.5 border-gray-200 focus:border-primary focus:ring-primary/10 rounded-lg text-xs py-2">
          ...
          <div v-if="Number(form.project_stage_id) === 2 && form.pushToPresales" class="space-y-3 p-4 rounded-xl border border-gray-200 bg-gray-50/50">
            <div>
              <InputLabel value="Pre-Sales Instructions" />
              <TextInput v-model="form.presale_actions" type="text" class="mt-1 block w-full text-xs" />
            </div>
            <div>
              <InputLabel value="Presale Flow Type" />
              <select v-model="form.presale_type" @change="form.populatePresaleOptions()" class="w-full mt-1 border-gray-200 focus:border-primary focus:ring-primary/10 rounded-lg text-xs py-2">
                <option value="1">Technical Meeting</option>
                <option value="2">Tenders</option>
                <option value="3">RFQ Response</option>
                <option value="4">Other</option>
              </select>
            </div>
          </div>

          <div v-if="form.presale_type" class="space-y-3 p-4 rounded-xl border border-gray-200 bg-gray-50/30">
            <div v-for="option in form.presale_options" :key="option.key" class="text-xs">
              <InputLabel :value="option.key.replace(/_/g, ' ')" />
              <TextInput v-if="['text', 'number'].includes(option.type)" v-model="option.value" :type="option.type" class="w-full mt-1 text-xs" />
              <TextInput v-else-if="option.type === 'date'" v-model="option.value" type="date" class="w-full mt-1 text-xs" />
              <TextInput v-else-if="option.type === 'time'" v-model="option.value" type="time" class="w-full mt-1 text-xs" />
              <div v-else-if="option.type === 'array'" class="mt-1 space-y-1.5">
                <div v-for="(guest, index) in option.value" :key="index" class="flex items-center gap-2">
                  <TextInput v-model="option.value[index]" type="text" class="w-full text-xs" :placeholder="`Guest Name ${index + 1}`" />
                  <SecondaryButton @click="option.value.splice(index, 1)" type="button" class="!py-1.5">Remove</SecondaryButton>
                </div>
                <SecondaryButton @click="option.value.push('')" type="button" class="!py-1.5">Add Guest</SecondaryButton>
              </div>
            </div>
          </div>

          <!-- Contract Section -->
          <div v-if="Number(form.project_stage_id) === 10" class="space-y-3 p-4 rounded-xl border border-gray-200 bg-gray-50/50 text-xs">
            <h4 class="font-bold text-gray-800">Contract & License Info</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <InputLabel value="Contract Name" />
                <TextInput v-model="form.contract_name" type="text" class="w-full mt-1 text-xs" />
              </div>
              <div>
                <InputLabel value="Contract Number" />
                <TextInput v-model="form.contract_number" type="text" class="w-full mt-1 text-xs" />
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <InputLabel value="Validity (Months)" />
                <TextInput v-model="form.contract_validity" type="text" class="w-full mt-1 text-xs" />
              </div>
              <div>
                <InputLabel value="Start Date" />
                <TextInput v-model="form.contract_start_date" type="text" class="w-full mt-1 text-xs" placeholder="dd/mm/yyyy" />
              </div>
              <div>
                <InputLabel value="End Date" />
                <TextInput v-model="form.contract_end_date" type="text" class="w-full mt-1 text-xs" placeholder="dd/mm/yyyy" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
              ...
              <div v-if="showLicenseTypeX">
                <InputLabel value="License Renewal type" />
            ...
            <div v-if="form.contract_license_type === 'Renewable'" class="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
              <div>
                <InputLabel value="Renewal Validity" />
                <TextInput v-model="form.contract_license_renewal_validity" type="text" class="w-full mt-1 text-xs" />
              </div>
              <div>
                <InputLabel value="Renewal Start" />
                <TextInput v-model="form.contract_license_renewal_start_date" type="text" class="w-full mt-1 text-xs" placeholder="dd/mm/yyyy" />
              </div>
              <div>
                <InputLabel value="Renewal End" />
                <TextInput v-model="form.contract_license_renewal_end_date" type="text" class="w-full mt-1 text-xs" placeholder="dd/mm/yyyy" />
              </div>
            </div>
          </div>
          ...
          <div v-if="Number(form.project_stage_id) === 1">
            ...
            <TextInput v-model="form.next_actions" type="text" class="w-full mt-1.5 text-xs" />
          </div>

          <div v-if="Number(form.project_stage_id) === 3" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <InputLabel value="Expected Sale Value" />
              <TextInput v-model="form.expected_sale_value" type="number" class="w-full mt-1.5 text-xs" />
            </div>
            <div>
              <InputLabel value="Projected Closure Month" />
              <TextInput v-model="form.projected_month_of_closure" type="text" class="w-full mt-1.5 text-xs" />
            </div>
          </div>

          <div v-if="Number(form.project_stage_id) === 4" class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            ...
            <div class="md:col-span-2">
              <InputLabel value="Expected Sale Value" />
              <TextInput v-model="form.expected_sale_value" type="number" class="w-full mt-1 text-xs" />
            </div>
            <div class="md:col-span-3">
              <InputLabel value="Expected Month of Closure" />
              <TextInput v-model="form.expected_month_of_closure" type="text" class="w-full mt-1 text-xs" />
            </div>
          </div>
          ...
          <div v-if="Number(form.project_stage_id) === 5" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <TextInput v-model="form.deal_amount" type="number" class="w-full mt-1.5 text-xs" />
            </div>
            <div>
              <InputLabel value="Margins Projection" />
              <TextInput v-model="form.margins_projection" type="text" class="w-full mt-1.5 text-xs" />
            </div>
            <div>
              <InputLabel value="Delivery Date" />
              <TextInput v-model="form.project_delivery_date" type="text" class="w-full mt-1.5 text-xs" />
            </div>
          </div>

          <div v-if="Number(form.project_stage_id) === 6">
            <InputLabel value="Reason for Losing" />
            <TextInput v-model="form.reason_for_losing" type="text" class="w-full mt-1.5 text-xs" />
          </div>

          <div v-if="Number(form.project_stage_id) === 7">
            <InputLabel value="Select Assigned Presale" />
            <select v-model="form.presale_assigned" class="w-full mt-1.5 border-gray-200 focus:border-primary focus:ring-primary/10 rounded-lg text-xs py-2">
          ...
          <div v-if="Number(form.project_stage_id) === 8" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <TextInput v-model="form.site_survey_comments" type="text" class="w-full mt-1.5 text-xs" />
            </div>
            <div>
              <InputLabel value="Site Survey Due Date" />
              <TextInput v-model="form.site_survey_due_date" type="text" class="w-full mt-1.5 text-xs" />
            </div>
          </div>
          ...
          <div v-if="Number(form.project_stage_id) === 9" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <TextInput v-model="form.reason_for_overdue" type="text" class="w-full mt-1.5 text-xs" />
            </div>
            <div>
              <InputLabel value="Next Steps" />
              <TextInput v-model="form.overdue_next_steps" type="text" class="w-full mt-1.5 text-xs" />
            </div>
          </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          ...
          <PrimaryButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing" @click.prevent="moveStage">
            Update Stage
          </PrimaryButton>
        </div>
      </template>
    ...
    <DialogModal :show="updateAccountModal" @close="closeUpdateAccountModal">
      ...
      <template #content>
        <div class="space-y-4">
          ...
          <div>
            <InputLabel value="Business Solution Title" />
            <TextInput v-model="editForm.business_name" type="text" class="w-full mt-1.5 block w-full text-xs" />
          </div>
          ...
          <div v-if="Number(editForm.project_stage_id) === 3" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <InputLabel value="Expected Sale Value" />
              <TextInput v-model="editForm.expected_sale_value" type="number" class="mt-1.5 block w-full text-xs" />
            </div>
