...
const editClient = (client) => {
    ...
    editForm.project_stage_id=client.project_stage_id;
    ...
    editForm.project_stage_id=client.latest_stage.project_stage_id;
    editForm.account_id = client.id;
    editForm.latest_stage_id = client.latest_stage.id;

    // editForm.expected_sale_value= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Expected Sale Value")).value;
    // editForm.expected_month_of_closure= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Estimated Month of Closure")).value;
    // editForm.projected_month_of_closure= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Projected Month of Closure")).value;
    // editForm.project_delivery_date= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Project Delivery Date")).value;
    // editForm.margins_projection= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Margins Projection")).value;
    // editForm.deal_amount= (JSON.parse(client.latest_stage.meta).find(item => item.key === "Deal Amount")).value;

    let metaData = JSON.parse(client.latest_stage.meta);

    // Helper function to find a key's value
    function getMetaValue(key) {
        let item = metaData.find(item => item.key === key);
        return item ? item.value : null; // Return the value if found, else null
    }
    ...
    editForm.probability_of_closure = getMetaValue("Closure Probability");

    console.log(editForm.deal_amount);
};
    ...
    <AppLayout :title="`${client.name} - Businesses`">
        ...
        <DialogModal :show="addingModal" @close="closeAddModal">
            ...
            <template #content>
                ...
                <div v-if="form.project_stage_id === 2" class="flex-1 ml-2 mt-3">
                        <input type="checkbox" id="pushToPresalesCheckbox" v-model="form.pushtopresales">
                        <label for="pushToPresalesCheckbox" class="pr-4"> Push To Presales</label>
                </div>
                <div class="mt-4" v-if="form.project_stage_id === 2 && form.pushtopresales === true" >
                    <TextInput v-model="form.presale_actions" type="text" class="mt-1 block w-full" placeholder="Pre-Sales Instructions" />
                ...
                <div class="mt-4" v-if="form.contract_license_type === 'Renewable' && form.project_stage_id === 10">
                ...
                <div class="mt-4" v-if="form.project_stage_id === 3 || form.project_stage_id === 4">
                    <TextInput v-model="form.expected_sale_value" type="number" class="mt-1 block w-full" placeholder="Expected Sale Value" />
                ...
                <div class="mt-4" v-if="form.project_stage_id === 5">
                    <TextInput v-model="form.deal_amount" type="number" class="mt-1 block w-full" placeholder="Deal Amount" />
                    <InputError :message="form.errors.deal_amount" class="mt-2" />

                    <TextInput v-model="form.margins_projection" type="number" class="mt-4 block w-full" placeholder="Margins Projection" />
        ...
        <DialogModal :show="updateStageModal" @close="closeUpdateStageModal">
            ...
            <template #content>
                ...
                <div class="mt-4" v-if="form.project_stage_id === 2 && form.pushtopresales">
                    <TextInput v-model="form.presale_actions" type="text" class="mt-1 block w-full" placeholder="Pre-Sales Instructions" />
                    <InputError :message="form.errors.presale_actions" class="mt-2" />
                </div>
        ...
        <DialogModal :show="updateAccountModal" @close="closeUpdateAccountModal">
            ...
            <template #content>
                <div class="mt-4">
                    <label for="">Select Business Type</label>
                    <select v-model="editForm.client_type_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select Business Type</option>
                        <option v-for="clienttype in clienttypes" :key="clienttype.id" :value="clienttype.id">{{ clienttype.name }}</option>
                    </select>
                    <InputError :message="form.errors.client_type_id" class="mt-2" />
                ...
                <div class="mt-4">
                    <label for="">Select Stage</label>
                    <select v-model="editForm.project_stage_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select stage</option>
                        <option v-for="projectstage in projectstages" :key="projectstage.id" :value="projectstage.id">{{ projectstage.name }}</option>
                    </select>
                    <InputError :message="editForm.errors.client_type_id" class="mt-2" />
                </div>


                <div class="mt-4" v-if="editForm.project_stage_id === 2 || editForm.project_stage_id === 3 || editForm.project_stage_id === 4 || editForm.project_stage_id === 5 || editForm.project_stage_id === 6 || editForm.project_stage_id === 7 || editForm.project_stage_id === 8 || editForm.project_stage_id === 9 || editForm.project_stage_id  === 10">
                    <TextInput v-model="editForm.business_name" type="text" class="mt-1 block w-full" placeholder="Business Name (How you Love it)" />
                    <InputError :message="editForm.errors.business_name" class="mt-2" />
                </div>

                <div class="mt-4">
                    <label for="">Select Solution Category</label>
                    <select v-model="editForm.solution_type_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select Solution Category</option>
                        <option v-for="solutiontype in props.solutiontypes" :key="solutiontype.id" :value="solutiontype.id">{{ solutiontype.solution_type_name }}</option>
                    </select>
                    <InputError :message="editForm.errors.solution_type_id" class="mt-2" />
                </div>

                <div class="mt-4">
                    <label for="">Select Solution</label>
                    <select v-model="editForm.solution_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select Solution</option>
                        <option v-for="solution in props.solutions" :key="solution.id" :value="solution.id">{{ solution.solution_name }}</option>
                    </select>
                    <InputError :message="editForm.errors.solution_id" class="mt-2" />
                </div>

                <div class="mt-4">
                    <label for="">Select Solution Subtype</label>
                    <select v-model="editForm.solution_sub_type_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select Solution Subtype</option>
                        <option v-for="subtype in props.solutionsubtypes" :key="subtype.id" :value="subtype.id">{{ subtype.solution_sub_type_name }}</option>
                    </select>
                    <InputError :message="editForm.errors.solution_subtype_id" class="mt-2" />
                </div>

                <div class="mt-4" v-if="editForm.project_stage_id === 5">
                    <TextInput v-model="editForm.deal_amount" type="number" class="mt-1 block w-full" placeholder="Deal Amount" />
                    <InputError :message="editForm.errors.deal_amount" class="mt-2" />

                    <TextInput v-model="editForm.margins_projection" type="text" class="mt-4 block w-full" placeholder="Margins Projection" />
                    <InputError :message="editForm.errors.margins_projection" class="mt-2" />

                    <TextInput v-model="editForm.project_delivery_date" type="text" class="mt-4 block w-full" placeholder="Project Delivery Date" />
                    <InputError :message="editForm.errors.project_delivery_date" class="mt-2" />
                </div>

                <div class="mt-4" v-if="editForm.project_stage_id === 4">
                    <label for="">Select Probability of Closure</label>
                    <select v-model="editForm.probability_of_closure" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>Select Probability of Closure</option>
                        <option value="Most Likely">Most Likely</option>
                        <option value="Likely">Likely</option>
                        <option value="Unlikely">Unlikely</option>
                    </select>
                    <InputError :message="editForm.errors.probability_of_closure" class="mt-2" />

                    <TextInput v-model="editForm.expected_sale_value" type="number" class="mt-1 block w-full" placeholder="Expected Sale Value"/>
                    <InputError :message="editForm.errors.expected_sale_value" class="mt-2" />

                    <TextInput v-model="editForm.expected_month_of_closure" type="text" class="mt-4 block w-full" placeholder="Expected Month of Closure" />
                    <InputError :message="editForm.errors.expected_month_of_closure" class="mt-2" />
                </div>

                <div class="mt-4" v-if="editForm.project_stage_id === 3">

                    <TextInput v-model="editForm.expected_sale_value" type="number" class="mt-1 block w-full" placeholder="Expected Sale Value"/>
                    <InputError :message="editForm.errors.expected_sale_value" class="mt-2" />

                    <TextInput v-model="editForm.projected_month_of_closure" type="text" class="mt-4 block w-full" placeholder="Projected Month of Closure" />
                    <InputError :message="editForm.errors.projected_month_of_closure" class="mt-2" />
                </div>

                <div class="mt-4" v-if="editForm.project_stage_id === 6">
                    <TextInput v-model="editForm.reason_for_losing" type="text" class="mt-1 block w-full" placeholder="Reason  for Losing" />
                    <InputError :message="editForm.errors.reason_for_losing" class="mt-2" />
                </div>
                ...
                <div v-for="contact in editForm.contact_information" class="mt-4 flex items-center">
                    ...
                    <select v-model="contact.tag"
                        class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                        <option value="" :disabled=true>CTYPE</option>
                        <option value="Key Contact">Key Contact</option>
                        <option value="Primary Contact">Primary Contact</option>
                        <option value="Secondary Contact">Secondary Contact</option>
                        <option value="Finance Contact">Finance Contact</option>
                ...
                <div v-for="(document, index) in editForm.documents" :key="document.id" class="mt-4 flex items-center">
                <div class="flex-1 mr-2">
                    <TextInput v-model="document.name" type="text" class="mt-1 block w-full" placeholder="Document Name" />
                </div>
                <div class="flex-1 ml-2">
                    <TextInput @change="handleFileChange2($event.target.files[0], index)" type="file" class="mt-1 block w-full" placeholder="Upload File" />
                </div>

                <div class="flex-1 ml-2">
                    <PrimaryButton class="ms-3 bg-red-500" @click="removeDocument2(document)">
                    <MdDelete />
                    </PrimaryButton>
                </div>
                </div>
                ...
                <div class="mt-4 flex justify-end">
                <PrimaryButton class="ms-3" @click="addDocuments2">
                    Add Document
                </PrimaryButton>
                </div>
            ...
            <template #footer>
                <SecondaryButton @click="closeUpdateAccountModal">
                    Cancel
                </SecondaryButton>

                <PrimaryButton class="ms-3" :class="{ 'opacity-25': form.processing }" :disabled="form.processing" @click.prevent="updateAccount">
                    Save
                </PrimaryButton>
            </template>
        </DialogModal>
